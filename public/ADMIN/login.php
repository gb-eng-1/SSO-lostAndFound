<?php
/**
 * Login page - only admin@ub.edu.ph / Admin (from database) can log in.
 * Wrong credentials show "Invalid Credentials." Success redirects to AdminDashboard.php.
 */
session_start();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (empty($email) || $password === '') {
    ...
    if ($error === '') {
      $stmt = $pdo->prepare('SELECT id, email, password_hash, name FROM admins WHERE LOWER(email) = LOWER(?) LIMIT 1');
      $stmt->execute([$email]);
      $admin = $stmt->fetch(PDO::FETCH_ASSOC);

      if (!$admin) {
        // Accept both admin@ub.edu.ph with Admin or admin123 password
        if (strtolower($email) === 'admin@ub.edu.ph' && ($password === 'Admin' || $password === 'admin123')) {
          $hash = password_hash('admin123', PASSWORD_DEFAULT);
          try {
            $ins = $pdo->prepare('INSERT INTO admins (email, password_hash, name, role) VALUES (?, ?, ?, ?)');
            $ins->execute(['admin@ub.edu.ph', $hash, 'Admin', 'Admin']);
          } catch (PDOException $e) {
            $ins = $pdo->prepare('INSERT INTO admins (email, password_hash, name) VALUES (?, ?, ?)');
            $ins->execute(['admin@ub.edu.ph', $hash, 'Admin']);
          }
          $admin = ['id' => (int) $pdo->lastInsertId(), 'email' => 'admin@ub.edu.ph', 'name' => 'Admin', 'role' => 'Admin'];
        } else {
          $error = 'Invalid Credentials.';
        }
  ...
  <div class="login-wrapper">
    <div class="login-box">
      ...
      <div style="text-align:center;margin-top:20px;font-size:13px;color:#6b7280;">
        ...
        <a href="../STUDENT/login.php" style="color:#8b0000;text-decoration:underline;">Student Login</a>
      </div>
    </div>
  </div>
  <?php if ($error): ?>
